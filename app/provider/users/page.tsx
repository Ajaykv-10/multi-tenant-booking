...
export default function ProviderUsersPage() {
  ...
  return (
    <div className="space-y-6">
                ...
                users.map((u) => (
                  <tr key={u.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-4 flex items-center gap-4">
                      {can("users", "edit") && (
                        <button onClick={() => openModal(u)} className="text-xs font-medium text-violet-600 hover:text-violet-800">Edit</button>
                      )}
                      {can("users", "delete") && (
                        <button onClick={() => handleDelete(u.id)} disabled={deleteId === u.id} className="text-xs font-medium text-red-500 hover:text-red-700 disabled:opacity-30">Delete</button>
                      )}
                    </td>
                  </tr>
                ))
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editTarget ? "Edit Staff" : "Add Staff"}>
